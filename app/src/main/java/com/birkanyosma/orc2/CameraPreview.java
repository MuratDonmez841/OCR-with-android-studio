...
public class CameraPreview implements SurfaceHolder.Callback {
	...
	// open cv için kullanılan sizelar
	int boxW = 100;
	int boxH = 100;
	int startX = 0;
	int startY = 0;
	...
	public void touchFocus(final Rect tfocusRect) {
		...
		// Görünüm genişlik ve yüksekliğini dönüştürüyoruz+/- 1000
		...
		try {// zoom için kullanıyoruz hata çıksa bile çok önemli değil program çalışır
	...
	private List<Rect> createAreaSign(int width, int height) {
		...
		// kesmek için ama kaldırdım isterseniz açabilirsiniz
		...
		protected void onDraw(Canvas canvas) {
			if (lstRectArea != null && lstRectArea.size() > 0) {
				for (Rect rect : lstRectArea) {
					canvas.drawRect(rect.left * .50F, rect.top * 1.0F, rect.right * .50F, rect.bottom * .9F,
							drawingPaint);//bu değerleri değiştirerek kırmızı alanı küçültebilirsiniz
				}
			}
		}
		public List<Rect> getCanvans(){
			return lstRectArea;
		}
	}
